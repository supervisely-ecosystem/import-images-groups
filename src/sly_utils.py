...
def remote_dir_exists(api: sly.Api, team_id: int, remote_directory: str) -> bool:
    ...
    files_infos = api.file.list(team_id, remote_directory)
    if len(files_infos) > 1:
        return True
    return False


def download_data_from_team_files(api: sly.Api, remote_path: str, save_path: str) -> str:
    ...
    if remote_dir_exists(api, g.TEAM_ID, remote_path):
        ...
        api.file.download_directory(g.TEAM_ID, remote_path, project_path)
        return project_path
    elif api.file.exists(g.TEAM_ID, remote_path):
        save_archive_path = os.path.join(
            save_path, get_file_name_with_ext(remote_path))
        api.file.download(g.TEAM_ID, remote_path, save_archive_path)
...
def process_images_groups(dataset_path: str, group_name_tag_meta: sly.TagMeta) -> Tuple[List[str], List[str], List[sly.Annotation]]:
...
def process_single_images(dataset_path: str) -> Tuple[List[str], List[str], List[sly.Annotation]]:
    ...
    images_paths = [os.path.join(images_dir, item) for item in os.listdir(images_dir) if
                    os.path.isfile(os.path.join(images_dir, item))]
